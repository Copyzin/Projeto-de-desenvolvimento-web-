...
export function useCourseSubjects(
  ...
  options?: { onlyCurrentStage?: boolean; studentId?: number },
) {
  const onlyCurrentStage = options?.onlyCurrentStage ?? false;
  const studentId = options?.studentId;
  return useQuery({
    queryKey: [api.courses.subjects.list.path, courseId, classSectionId, onlyCurrentStage, studentId],
    queryFn: async () => {
      ...
      if (onlyCurrentStage) url.searchParams.set("onlyCurrentStage", "true");
      if (studentId) url.searchParams.set("studentId", String(studentId));
