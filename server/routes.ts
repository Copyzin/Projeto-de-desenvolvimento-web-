...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.patch(api.users.updateProfile.path, requireAuth, async (req, res) => {
    try {
      ...
      const nickname = input.nickname && input.nickname.length > 0 ? input.nickname : null;
      const updatedUser = await storage.updateUser(user.id, { nickname });
      return res.json(sanitizeUser(updatedUser));
    } catch (error) {
      return handleRouteError(res, error);
    }
  ...
  app.put(api.users.updatePreferences.path, requireAuth, async (req, res) => {
    try {
      const input = api.users.updatePreferences.input.parse(req.body);
      const user = getAuthUser(req);

      const updatedUser = await storage.updateUser(user.id, { preferences: input });
      return res.json(sanitizeUser(updatedUser));
    } catch (error) {
      return handleRouteError(res, error);
    }
  ...
  app.get(api.courses.subjects.list.path, requireAuth, async (req, res) => {
    ...
    const onlyCurrentStage = req.query.onlyCurrentStage === "true";
    ...
    // Alunos recebem status academico personalizado baseado em suas proprias notas.
    // Admins/professores podem consultar o status de um aluno especifico via query.
    let studentId: number | undefined;
    if (user.role === "student") {
      studentId = user.id;
    } else {
      const queryStudentId = parseOptionalPositiveInt(req.query.studentId);
      if (queryStudentId) studentId = queryStudentId;
    }

    const subjects = await storage.getCourseSubjects(
      courseId,
      classSectionId,
      currentStageNumber,
      onlyCurrentStage,
  ...
  app.get(api.lessonSchedules.teacherLoads.path, requireRoles("admin"), async (_req, res) => {
    try {
      const loads = await storage.getTeacherClassLoads();
      return res.json(loads);
    } catch (error) {
      return handleRouteError(res, error, "Erro ao buscar carga dos professores");
    }
  });

  app.get(api.lessonSchedules.teacherBusySlots.path, requireRoles("admin"), async (req, res) => {
    try {
      const input = api.lessonSchedules.teacherBusySlots.input.parse(req.query);
      const busy = await storage.getTeacherBusySlots(
        input.academicTermId,
        input.period,
        input.excludeClassSectionId,
      );
      return res.json(busy);
    } catch (error) {
      return handleRouteError(res, error, "Erro ao buscar horarios ocupados");
    }
  });
  ...
  app.patch(api.classSections.updateStage.path, requireRoles("admin"), async (req, res) => {
    try {
      ...
      const input = api.classSections.updateStage.input.parse(req.body);
      const section = await storage.updateClassSectionStage(sectionId, input.currentStageNumber);
      return res.json(section);
    } catch (error) {
      return handleRouteError(res, error, "Erro ao atualizar etapa da turma");
    }
  });

  app.get(api.mySchedule.student.path, requireRoles("student"), async (req, res) => {
    try {
      const user = getAuthUser(req);
      const schedule = await storage.getStudentActiveSchedule(user.id);
      return res.json(schedule);
    } catch (error) {
      return handleRouteError(res, error, "Erro ao buscar horario do aluno");
    }
  });

  app.get(api.mySchedule.teacher.path, requireRoles("teacher"), async (req, res) => {
    try {
      const user = getAuthUser(req);
      const schedule = await storage.getTeacherActiveSchedule(user.id);
      return res.json(schedule);
    } catch (error) {
      return handleRouteError(res, error, "Erro ao buscar horario do professor");
    }
  });

  app.get(api.myDisciplines.list.path, requireAuth, async (req, res) => {
    try {
      const user = getAuthUser(req);

      if (user.role === "student") {
        const studentItems = await storage.getStudentDisciplines(user.id);
        return res.json({ role: user.role, studentItems, teacherItems: [] });
      }

      if (user.role === "teacher") {
        const teacherItems = await storage.getTeacherDisciplines(user.id);
        return res.json({ role: user.role, studentItems: [], teacherItems });
      }

      // Admin nao possui disciplinas proprias; o painel dele usa outro bloco.
      return res.json({ role: user.role, studentItems: [], teacherItems: [] });
    } catch (error) {
      return handleRouteError(res, error, "Erro ao buscar disciplinas");
    }
  });

  app.get(api.teacherLessons.listByDate.path, requireRoles("teacher"), async (req, res) => {
    try {
      const user = getAuthUser(req);
      const input = api.teacherLessons.listByDate.input.parse(req.query);
      const data = await storage.getTeacherLessonsByDate(user.id, input.date);
      return res.json(data);
    } catch (error) {
      return handleRouteError(res, error, "Erro ao buscar aulas do dia");
    }
  });

  app.post(api.teacherLessons.saveRecord.path, requireRoles("teacher"), async (req, res) => {
    try {
      const user = getAuthUser(req);
      const input = api.teacherLessons.saveRecord.input.parse(req.body);
      const saved = await storage.saveTeacherLessonRecord(user.id, input);
      return res.json(saved);
    } catch (error) {
      return handleRouteError(res, error, "Erro ao registrar aula");
    }
  });
  ...
  app.get(api.finances.summary.path, requireAuth, async (req, res) => {
    try {
      const user = getAuthUser(req);
      if (user.role !== "student") {
        return res.status(403).json({ message: "Apenas alunos possuem boletos de mensalidade." });
      }
      const summary = await storage.getStudentFinanceSummary(user);
  ...
  app.get(api.finances.adminOverview.path, requireAuth, async (req, res) => {
    try {
      const user = getAuthUser(req);
      if (user.role !== "admin") {
        return res.status(403).json({ message: "Acesso restrito a administradores." });
      }
      const overview = await storage.getAdminFinanceOverview();
